...
const CustomNavLink = ({ children, to, className }: { children: React.ReactNode; to: string; className: string }) => {
  return (
    <NavLink
      to={to}
      className={className}
...
const Navbar = () => {
  const [_, setStickyClass] = useState('relative pt-8')
  ...
  const stickNavbar = () => {
    if (window !== undefined) {
      ...
      windowHeight > 80
        ? setStickyClass('!fixed inset-x-0 z-50 bg-white duration-100 text-[#243c5a]')
