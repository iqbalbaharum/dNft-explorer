...
const StatusTextBadge = (status: number, error_text?: string) => {
  ...
  switch (status) {
    case 0:
      return (
        <span className="inline-flex hover:shadow-md cursor-pointer text-xs items-center justify-center border border-gray-300 rounded-md bg-gray-100 p-2 text-gray-700">
          PENDING
        </span>
      )
    case 1:
      return (
        <span className="inline-flex hover:shadow-md cursor-pointer text-xs items-center justify-center border border-gray-300 rounded-md bg-emerald-100 p-2 text-emerald-700">
    ...
    case 2:
      return (
        <a data-tooltip-id="my-tooltip" data-tooltip-content={error_text}>
          <span className="inline-flex hover:shadow-md cursor-pointer text-xs items-center justify-center bg-gray-100 border border-gray-300 rounded-md p-2 text-red-700">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5">
              <path
                fill-rule="evenodd"
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          let error_text = ''
          if (txs) {
            error_text = `${txs[row.index].status}` ?? ''
          }
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Link to={`/tx/${row.getValue('hash')}`}>
            <p className="truncate">{row.getValue('hash')}</p>
          </Link>
        ),
        size: 100,
      },
      {
        ...
        cell: ({ row }) => {
          return <div className="text-center">{MethodTextBadge(row.getValue('method'))}</div>
        },
        size: 60,
      },
      {
        ...
        cell: ({ row, cell }) => {
          ...
          return (
            <>
              ...
              <Badge
                className="bg-indigo-100 text-black text-xs font-medium px-4 py-1 rounded w-20 overflow-hidden text-ellipsis"
                text={row.original.public_key}
              />
            </>
          )
        },
      ...
      },
      {
        ...
        cell: ({ row }) => <i>{row.getValue('status')}</i>,
      },
      {
      ...
      <section className="rounded-xl w-full border border-gray-200 bg-white shadow-md flex items-center justify-center mt-5 md:mx-0">
        <div className="w-full  relative block border border-gray-100 p-2 shadow-sm text-left">
          <div className="p-2 sm:items-center sm:justify-between text-left ">
            ...
            <div className="text-sm text-gray-600">Total {txs?.length ?? 0} transactions</div>
          </div>
